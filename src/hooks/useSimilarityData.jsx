...
export default function useSimilarityData({
    ...
    return useMemo(() => {
        ...
        const hasAllSimilarityData = selectedBookIds.every((id) => similarityCache[id]);
        ...
        selectedBookIds.forEach((id) => {
            stats[id] = { total: similarityCache[id]?.length ?? 0, removed: 0, shown: topN };
        });
        ...
        for (const bookId of selectedBookIds) {
            for (const item of similarityCache[bookId] || []) {
                if (!termDataMap.has(item.term)) {
                    termDataMap.set(item.term, { term: item.term, byBook: {}, mean: 0, sortable: 0 });
                }
        ...
        for (const row of termDataMap.values()) {
            ...
            const meetsMinCount = selectedBookIds.every((id) => (row.byBook[id]?.n ?? 0) >= 10);
